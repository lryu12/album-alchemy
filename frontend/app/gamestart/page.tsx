...
const Page = () => {
  ...
  function GenerateRandomQuestion() {
    ...
    const questionAlbumIncorrectRandomPhoto1 =
      albums[randomNums[1]].images[1].url;
    const questionAlbumIncorrectRandomPhoto2 =
      albums[randomNums[2]].images[1].url;
    const questionAlbumIncorrectRandomPhoto3 =
      albums[randomNums[3]].images[1].url;
    const questionAlbumIncorrectRandomPhoto4 =
      albums[randomNums[4]].images[1].url;
    const images = [
        ...
        <div className="flex flex-row">
          {shuffledImages.map((image, index: number) => (
            <img
              key={index}
              src={image.src}
              onClick={() => checkCorrect(image.isCorrect)}
            />
          ))}
        </div>
      </div>
